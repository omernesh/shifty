...
// 7. schedule_audit row (to_state='soldier_created') via the AuditWrite pattern
//    (now co-located in this same plugin at ./AuditWrite.js after Phase 02 hotfix 02-11)
...
async function CreateSoldier({ request, connection }) {
  ...
  const result = await withTenantTx(connection, tenant_id, async (trx) => {
    ...
    // Step 6: audit row (matches AuditWrite payload shape; co-located in this plugin).
