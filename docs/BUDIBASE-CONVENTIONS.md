...
| **2 — Query** | Every domain-table SQL filter includes `WHERE tenant_id = '{{ Current User.shiftyTenantId }}'::uuid` | **ACTIVE — newly load-bearing as the top defense.** Enforced by CI gate (`tools/check-bb-queries.mjs`) |
...
**Field name decision.** Budibase user docs already have a built-in `tenantId` field that Budibase populates with the *workspace* tenant (string `"default"` in CE single-tenant mode) and uses for its own request routing. We use a separate field — **`shiftyTenantId`** — to hold the Shifty domain tenant UUID. This avoids overwriting Budibase's internal routing field. The decision was made during W0-02 execution on 2026-05-18 after the dump-configs probe revealed the collision.

**User docs are schemaless.** Empirically verified on 2026-05-18 against Budibase 3.38.4 CE: there is NO `customUserSchema` config doc type in the worker (valid `/api/global/configs/<type>` types are exactly: `settings, account, smtp, google, oidc, logos_oidc, scim, ai, recaptcha, translations`). Budibase user docs accept arbitrary top-level fields without any prior declaration — the "custom field" mechanism in the Builder UI is a form-rendering hint, not a schema-validation gate.

**Procedure (autonomous, headless):**

1. **Populate** `shiftyTenantId` on each user record via `POST /api/global/users` (Builder UI's update-user endpoint). The field persists without any prior declaration. Use `tools/budibase-cli/src/apply-tenantid.mjs`.
2. **For new users created via invite-redemption**, the Automation `InviteRedemption_PopulateTenantId` (created by W0-02; spec at `tools/budibase-cli/fixtures/automation-invite-redemption.json`) resolves `tenant_id` from the invite code, INSERTs an `app_user` row, and PATCHes the Budibase user record with `shiftyTenantId`.
3. **Reference in every Query body** as `{{ Current User.shiftyTenantId }}` — wrap in single quotes + cast `::uuid` (e.g. `WHERE tenant_id = '{{ Current User.shiftyTenantId }}'::uuid`).
4. **The Layer-2 CI gate** (`tools/check-bb-queries.mjs`) verifies the canonical pattern is present on every domain-table query. The gate's `TENANT_FILTER_PATTERN` was updated 2026-05-18 to match `Current User.shiftyTenantId`.

**Alternative (interactive, Builder UI):** the same field can be set manually via Builder UI → Settings → Users → edit a user → set `shiftyTenantId` on the form (assuming a "custom field" Hint is added to the user-table form — optional, since the field persists schemaless). Use this path for one-off manual fixes; use the headless path for repeatable / scripted work.

**Caveats:**
- `{{ Current User.shiftyTenantId }}` is parameterized (not textual substitution), so SQL injection risk is bounded.
- DO NOT overwrite the built-in `tenantId` field — it's load-bearing for Budibase's own routing. Use `shiftyTenantId` for Shifty domain tenancy.
...
| Read queries | Authored in Builder UI → Queries (PostgreSQL data source) OR via `tools/budibase-cli/` for scripted/repeatable creation. MUST include `WHERE tenant_id = '{{ Current User.shiftyTenantId }}'::uuid` on domain tables — CI gate enforces, regardless of which surface authored the query. |
